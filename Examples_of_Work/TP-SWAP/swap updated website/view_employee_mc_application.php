...
<body>
	...
	<div class="table_container">
        ...
        $get_mc_info = $con->query('SELECT * FROM mc_application 
WHERE mc_id=' . $mc_id);
        
        //displaying mc details
        $mc = $get_mc_info->fetch_assoc();
        $get_mc_info->close();
        $get_employee_name = $con->query('SELECT name FROM employee_information
WHERE employee_id=' . $mc['employee_id']);
        $name = $get_employee_name->fetch_row()[0];
        $get_employee_name->close();
        
        echo "<tr><td>Employee ID:</td><td>" . $mc['employee_id'] . "</td></tr>";
        echo "<tr><td>Name:</td><td>" . $name . "</td></tr>";
        echo "<tr><td>Start Date:</td><td>" . $mc['start_date'] . "</td></tr>";
        echo "<tr><td>End Date:</td><td>" . $mc['end_date'] . "</td></tr>";
        echo "<tr><td>MC Picture:</td><td>
<img style='width: 500px; height: 500px;' src='data:image/jpeg;base64," . base64_encode($mc['mc_picture']) . "' 
alt='MC Picture'></td></tr>";
        echo "<tr><td>Status:</td><td>" . $mc['status'] . "</td></tr>";
        ...
        document.querySelector('.button_container').style.display = '<?php echo ($mc['status'] == 'approved' || $mc['status'] == 'rejected') ? 'none' : 'block'; ?>';
    	...
    	function open_modal(action) {
    		...
    		? 'Are you sure you want to approve this MC?' 
    		: 'Are you sure you want to reject this MC?';
    	...
    	document.getElementById('confirm_button').addEventListener('click', function() {
    		...
    		if (current_action === 'approve') {
    			window.location.href = 'approve_mc.php';
    		} else if (current_action === 'reject') {
    			window.location.href = 'reject_mc.php';
    		}
    	...
    	window.addEventListener('beforeunload', function() {
    		// Perform an action before the user mcs the page
    		window.location.href = 'unset_session_var.php?var=' + encodeURIComponent('mc_id');
    	});
